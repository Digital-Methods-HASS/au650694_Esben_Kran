...
*There's no homicide number*.

```{r Inspecting the top of the dataframe}
head(Western_Europe)
```
...
```{r Scatter plot of all data points}
...
```{r Smoothed trends per region}
df %>% 
  ggplot() +
  aes(Year, homicides, color = Entity) +
  geom_point() +
  geom_smooth(formula = 'y ~ x', se = F) +
  theme_minimal() +
  labs(
    title = "Homicides per 100,000 by year",
    y = "Homicides",
    subtitle = "Downward trends are present in all regions of the dataset"
  )
...
I believe it already is but here is the above graph with the facet wrap.
```{r Faceted smoothed legend right}
df %>% 
  ggplot() + 
  aes(x = Year, y = homicides, color = Entity) +
  geom_point() +
  facet_wrap(~ Entity, nrow = 3) +
  geom_smooth(se = F) +
  labs(x = "Year",
...
```{r Faceted smoothed legend bottom}
df %>% 
  ggplot() + 
  aes(x = Year, y = homicides, color = Entity) +
  geom_point() +
...
I rewrote the YAML header to include the current date in a proper format and output an HTML with a table of content (toc).
```
date: "`r format(Sys.time(), '%d %B, %Y')`"
output: 
  html_document:
